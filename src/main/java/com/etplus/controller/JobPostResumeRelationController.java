...
public class JobPostResumeRelationController {
  ...
  @GetMapping
  public CommonResponse<List<JobPostResumeRelationVO>> getJobPostResumeRelations(
      @AuthUser({RoleType.TEACHER, RoleType.ACADEMY}) LoginUser loginUser) {
    List<JobPostResumeRelationVO> result = jobPostResumeRelationService
        .getAllJobPostResumeRelations(loginUser.roleType(), loginUser.userId());
